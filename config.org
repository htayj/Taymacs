...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar emo-mode-map
    (let ((map(make-sparse-keymap)))
       ...
       (define-key map (kbd "M-s") 'backward-char)
       (define-key map (kbd "M-d") 'next-line)
       (define-key map (kbd "M-e") 'previous-line)
       (define-key map (kbd "M-f") 'forward-char)
       (define-key map (kbd "M-r") 'end-of-line)
       (define-key map (kbd "M-w") 'beginning-of-line-text) ;;todo dwim

       (define-key map (kbd "M-S-s") 'backward-word)
       (define-key map (kbd "M-S-e") 'golden-ratio-scroll-screen-up)
       (define-key map (kbd "M-S-d") 'golden-ratio-scroll-screen-down)
       (define-key map (kbd "M-S-f") 'forward-word)  
       (define-key map (kbd "M-S-r") 'forward-list) 
       (define-key map (kbd "M-S-w") 'backward-list) 
