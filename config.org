...
#+Based_on_config_by: Dawid 'daedreth' Eckert
#+LANGUAGE: en
#+OPTIONS: num:nil
...
#+END_SRC
* Logic behind keybindings
** Modifier keys
*** Ctrl
/ do, atomic, common, simple /

Ctrl-[key] should do a single action, no chaining. For the most common actions that manipulate buffers/files in buffers and the cursor.

Do small things (see Shift)

Typically is the base level of a modifier chain but can mean do atomically if it is not

*** Meta (Alt) 
/ meta-actions, do more complicated, meta-control, opposite, deep /

Meta is for meta-actions that are slightly dependent on code, and actions that require chaining of keys

when modifying other modifiers, it means either alter or deeper

*** Shift
-- More, bigger, whole, all, expand --

Shift modifies other modifers to make them do bigger things

*** Super (windows key)
-- WM, windows, frames, buffer manipulation --

Super is used for movement around emacs and other wm-like features. It is a conceptual level above meta and control. it does not manipulate what is in the buffers, at the cursor, or the state of files

It manipulates the state of the editor/os itself

*** Hyper (you should rewrite its purpose for yourself) 
-- OS level, hotkeys --

only does custom stuff that is highly suited to my specific needs 

*** gigA (Emacs Alt internally, )
-- generative, fancy stuff --

todo - generative actions


** Leader Keys
Todo
*** viper (mode/state switch)
*** Option? (menu key)

** Design Patterns
*** Text-based movement
hjkl represent text based movement - vim style

*** concept based movement (Orgy mode)
a section (function, list, entry, variable, object, statement) of code is a body. There are bodies inside of bodies. Bodies next to each other are stacked sUperior-Inferior

when traversing inward choose the closest to cursor or the middle if no cursor
bodies also have non spatial relationships with other bodies? declaration/spawner, parent, child, kind
multiple bodies at a time? rainbow colored
highlight in varrying shades of alpha
 -highlight current body, superficial, closest deep
...
todo terms for rotations
todo enlongate shoRten
**** a - anterior
/ front, beginning, head /
header, definition, label, beginning
internally head-wise
**** p - posterior
/ end, tail, feet /
internally tail-wise
**** s - superficial
/ outward, surface /
**** d - deeper
/ inward, core-wise /
**** u - superior
/above/
externally above body
**** i - inferior 
/below/
externally below body
**** skin
**** g - finGer/heh
the thing that performs the action
**** fascia delimiters?
**** go back to last selected body

*** concept based action (Orgy mode)
**** duplicate
**** chaNge
**** c - Copy/kill/yank
**** v - paste/yank (riVet)
...
**** r - shoRten/reduce
**** w - wiggle
move around (sup context)
**** + - increment/rot+
**** - - decrement/rot-
**** stroke?
highlight
**** j - insert outside body (follow inferior or superior)
put new body inferior or superior
**** b - spawn a body
b again for blank, select direction, select kind
** 
...
#+BEGIN_SRC emacs-lisp
(require 'exwm-randr)
(setq exwm-randr-workspace-output-plist '(0 "HDMI-0" 1 "DP-4"))
...
** super modifier for windows and buffers (snav)
#+BEGIN_SRC emacs-lisp
(defvar snav-mode-map
  (let ((map(make-sparse-keymap)))
     
     (define-key map (kbd "s-n") 'ibuffer)
     (define-key map (kbd "s-m") 'next-buffer)
     (define-key map (kbd "s-,") 'previous-buffer)
     (define-key map (kbd "s-/") 'kill-current-buffer) 
     ...
     (define-key map (kbd "s-s h") 'split-window-right)
     (define-key map (kbd "s-s j") 'split-window-below)
     (define-key map (kbd "s-s k") 'split-and-follow-horizontally)
     (define-key map (kbd "s-s l") 'split-and-follow-vertically)

     ;;(define-key map (kbd "s-S-h") 'backward-word)
     ;;(define-key map (kbd "s-S-j") 'golden-ratio-scroll-screen-up)
     ;;(define-key map (kbd "s-S-k") 'golden-ratio-scroll-screen-down)
     ;;(define-key map (kbd "s-S-l") 'forward-word) 

   map)
"snav-mode keymap.")

(define-minor-mode snav-mode
  "navigation of frames, windows, and buffers like default settings for tiling wms"
  :init-value t
  :lighter "snav"
...
;; The keymaps in `emulation-mode-map-alists' take precedence over
;; `minor-mode-map-alist'
(add-to-list 'emulation-mode-map-alists `((snav-mode . ,snav-mode-map)))
